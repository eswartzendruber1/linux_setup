...
(defconst vpp-defun-re
  (eval-when-compile (vpp-regexp-words `("macromodule" "module" "class" "program" "interface" "package" "primitive" "config"))))
...
BASEIND is the base indent to offset everything."
  ...
  (let ((pos (point-marker))
	(lim (save-excursion
	       ...
	       (vpp-re-search-backward "\\(\\<begin\\>\\)\\|\\(\\<module\\>\\)\\|\\(\\<task\\>\\)" nil 'move)
	       (point)))
	(ind)
	(val)
	(m1 (make-marker)))
	 ...
	 (with-current-buffer (find-file-noselect filename)
	   (save-excursion
	     ...
	     (while (and
		     ...
		     (vpp-re-search-forward-quick "\\<\\(module\\|interface\\|program\\)\\>" nil t)
